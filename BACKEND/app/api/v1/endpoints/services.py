...
async def read_service_requests(
    ...
    result = await db.execute(select(models.ServiceRequest).offset(skip).limit(limit))
    return result.scalars().all()

@router.get("/service-requests/me", response_model=List[schemas.ServiceRequest])
async def read_my_service_requests(
    db: AsyncSession = Depends(deps.get_db),
    skip: int = 0,
    limit: int = 100,
    current_user: models.User = Depends(deps.get_current_active_user),
    ...
    """
    result = await db.execute(select(models.ServiceRequest).filter(models.ServiceRequest.user_id == current_user.id).offset(skip).limit(limit))
    return result.scalars().all()
...
async def update_service_request_status(
    *,
    ...
    if not current_user.is_superuser and current_user.role != models.UserRole.SERVICIO_TECNICO:
        raise HTTPException(status_code=400, detail="Not enough privileges")
    result = await db.execute(select(models.ServiceRequest).filter(models.ServiceRequest.id == id))
...
async def read_warranty_registrations(
    db: AsyncSession = Depends(deps.get_db),
    skip: int = 0,
    limit: int = 100,
    type: str = "standard",
    ...
    query = select(models.WarrantyRegistration)
    if type:
...
async def delete_warranty_registration(
    *,
    db: AsyncSession = Depends(deps.get_db),
    id: int,
    current_user: models.User = Depends(deps.get_current_active_superuser),
    ...
    """
    result = await db.execute(select(models.WarrantyRegistration).filter(models.WarrantyRegistration.id == id))
    obj = result.scalars().first()
    if not obj:
