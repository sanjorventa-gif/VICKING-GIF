...
async def read_carousel_items(
    ...
    """
    items = await crud.carousel.get_multi(db, skip=skip, limit=limit)
    ...
    items = list(items) # Convert sequence to list for sorting
    items.sort(key=lambda x: x.order)
    return items

@router.get("/{id}", response_model=schemas.CarouselItem)
async def read_carousel_item(
    *,
    db: AsyncSession = Depends(deps.get_db),
    id: int,
) -> Any:
    ...
    """
    item = await crud.carousel.get(db=db, id=id)
    if not item:
...
async def create_carousel_item(
    *,
    ...
    """
    item = await crud.carousel.create(db=db, obj_in=item_in)
    return item
...
async def update_carousel_item(
    *,
    ...
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    item = await crud.carousel.update(db=db, db_obj=item, obj_in=item_in)
    return item
...
async def delete_carousel_item(
    *,
    ...
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    item = await crud.carousel.remove(db=db, id=id)
    return item
