...
async def read_history(
    ...
    """
    history = await crud.history.get_multi(db, skip=skip, limit=limit)
    return history
...
async def create_history(
    *,
    ...
    """
    history = await crud.history.create(db=db, obj_in=history_in)
    return history
...
async def reorder_history(
    *,
    ...
    for item in ordered_history:
        entry = await crud.history.get(db=db, id=item["id"])
        if entry:
            updated_entry = await crud.history.update(db=db, db_obj=entry, obj_in={"order": item["order"]})
...
async def update_history(
    *,
    ...
    """
    history = await crud.history.get(db=db, id=id)
    if not history:
        raise HTTPException(status_code=404, detail="History entry not found")
    history = await crud.history.update(db=db, db_obj=history, obj_in=history_in)
    return history
...
async def delete_history(
    *,
    ...
    if not history:
        raise HTTPException(status_code=404, detail="History entry not found")
    history = await crud.history.remove(db=db, id=id)
    return history
