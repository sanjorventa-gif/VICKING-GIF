...
async def read_news(
    ...
    Retrieve news.
    """
    news = await crud.news.get_multi(db, skip=skip, limit=limit)
    return news
...
async def create_news(
    *,
    ...
    """
    news = await crud.news.create(db=db, obj_in=news_in)
    return news
...
async def update_news(
    *,
    ...
    Update a news item.
    """
    news = await crud.news.get(db=db, id=id)
    if not news:
        raise HTTPException(status_code=404, detail="News item not found")
    news = await crud.news.update(db=db, db_obj=news, obj_in=news_in)
    return news
...
async def read_news_item_by_slug(
    *,
    db: AsyncSession = Depends(deps.get_db),
    slug: str,
) -> Any:
    ...
    """
    result = await db.execute(select(models.News).filter(models.News.slug == slug))
...
async def delete_news(
    *,
    ...
    if not news:
        raise HTTPException(status_code=404, detail="News item not found")
    news = await crud.news.remove(db=db, id=id)
    return news
